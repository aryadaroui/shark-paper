...
% \author{\IEEEauthorblockN{Sainesh Karan\textsuperscript{1}, Emily Messe\textsuperscript{2}, Dr. Yu Yang\textsuperscript{3}, Dr.Hen-Guel Yeh\textsuperscript{1}, Dr. Chris Lowe\textsuperscript{2}, Dr. Wenlu Zhang\textsuperscript{4}, Arya Daroui\textsuperscript{1}}
% \IEEEauthorblockA{
% \textsuperscript{1}\textit{Dept. of Electrical Engineering, California State University, Long Beach, CA - 90840}\newline
% \textsuperscript{2}\textit{Dept. of Biological Sciences, California State University, Long Beach, CA- 90840\newline
% \textsuperscript{3}\textit{Dept. of Chemical Engineering, California State University, Long Beach, CA- 90840\newline}\textsuperscript{4}\textit{Dept. of Computer Science,California State University, Long Beach, CA- 90840\newline}
% }}}
...
\begin{figure}[h]
	...
	\caption{Breakdown of acceleration data recorded at 25 Hz. (a) Static acceleration represents the body position and orientation of the shark due to gravity. (b)Dynamic acceleration represents the body movements of the shark.}
	\label{accel}
\end{figure}
...
Another parameter introduced to improve the performance of classifier is the Cost Matrix. It is a square matrix, denoted by $cost (i, j)$, which represents the penalty of mis-classifying a data belonging to class $i$ to class $j$.  Naturally all diagonal elements of the Cost Matrix are zero since there no penalty for correct classification. 

The algorithm classifies the new data sample by minimizing the expected cost given by equation 2.

\begin{equation}
	y = \mathrm{arg} \mathrm{min}_{y = 1 \ldots n} \sum^n_{j=1} P(i|x)C(i|j)\qquad P(i|x) = \frac{k_i}{k}
\end{equation}

where $y$ is the predicted classification, $n$ is the number of classes and $P(i|x)$ is the condiional probability of class $i$ for observation $x$ and $C(i|j)$ is the user defined cost of classifying an observation as $i$ when its true class is $j$.

In our study, we developed the cost matrix by trial and error method using the time domain data. We ran multiple iterations of the program and updated the elements of the cost matrix in increments of 0.25 until we were able to strike a balance between accuracy's for all the behavior classes. The final cost matrix developed is shown below:
...
The results for the experiment without using the cost matrix are shown in Table IV:
